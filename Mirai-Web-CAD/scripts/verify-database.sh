...
IFS=: read -r database table_count project_count drawing_count version_count audit_count <<<"$verification"

if [[ "$table_count" != "8" || "$project_count" != "1" || "$drawing_count" != "1" || "$version_count" != "1" || "$audit_count" != "1" ]]; then
  echo "database verification failed: tables=$table_count project=$project_count drawing=$drawing_count version=$version_count audit=$audit_count" >&2
  exit 1
fi
