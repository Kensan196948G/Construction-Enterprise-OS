...
const sitePins = [
  { id: 1, name: '品川タワー新築工事', lat: 35.6285, lng: 139.7387, status: 'active', workers: 42, alerts: 0 },
  { id: 2, name: '横浜分譲マンション建設', lat: 35.4437, lng: 139.6380, status: 'active', workers: 28, alerts: 0 },
  { id: 3, name: '大田区道路改良工事', lat: 35.5614, lng: 139.7161, status: 'active', workers: 15, alerts: 0 },
  { id: 4, name: '川崎物流センター建設', lat: 35.5310, lng: 139.7025, status: 'alert', workers: 33, alerts: 3 },
  { id: 5, name: '新宿再開発ビル工事', lat: 35.6895, lng: 139.6917, status: 'planning', workers: 0, alerts: 0 },
]

const nearbyAlerts = [
  { type: 'weather', message: '強風警報 — 高所作業中止を推奨 (14:00〜18:00)', severity: 'warning', site: '品川タワー' },
  { type: 'equipment', message: '重機振動異常 — センサーS-010 閾値超過', severity: 'alert', site: '川崎物流センター' },
  { type: 'traffic', message: '搬入ルート渋滞 — 国道15号線 事故処理中', severity: 'info', site: '横浜マンション' },
]

const equipment = [
  { id: 'EQ-001', name: 'タワークレーン TC-200', site: '品川タワー新築工事', status: 'operating', operator: '木村 一郎' },
  { id: 'EQ-002', name: '油圧ショベル ZX350', site: '川崎物流センター建設', status: 'alert', operator: '加藤 二郎' },
  { id: 'EQ-003', name: 'コンクリートポンプ車', site: '横浜分譲マンション建設', status: 'standby', operator: '松本 三郎' },
  { id: 'EQ-004', name: 'バックホウ PC200', site: '大田区道路改良工事', status: 'operating', operator: '井上 四郎' },
]
    ...
    <div className="p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-gray-500 text-sm">工事現場・重機位置・安全区域のリアルタイム地図管理</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="relative rounded-xl border border-gray-200 bg-white overflow-hidden" style={{ height: '480px' }}>
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-blue-50 via-green-50 to-gray-100">
              ...
              <svg className="absolute inset-0 w-full h-full" xmlns="http://www.w3.org/2000/svg">
                <line x1="0" y1="240" x2="100%" y2="240" stroke="#e2e8f0" strokeWidth="6" />
                <line x1="0" y1="200" x2="100%" y2="300" stroke="#e2e8f0" strokeWidth="4" />
                <line x1="200" y1="0" x2="200" y2="100%" stroke="#e2e8f0" strokeWidth="6" />
                <line x1="400" y1="0" x2="350" y2="100%" stroke="#e2e8f0" strokeWidth="4" />
                <line x1="0" y1="350" x2="100%" y2="380" stroke="#e2e8f0" strokeWidth="3" />
                {/* Road labels */}
                <text x="10" y="235" fill="#94a3b8" fontSize="10">国道15号線</text>
                <text x="205" y="120" fill="#94a3b8" fontSize="10">第一京浜</text>
              </svg>
                ...
                return (
                  ...
                  >
                    <div className={`flex h-8 w-8 items-center justify-center rounded-full shadow-lg ${
                      pin.status === 'alert' ? 'bg-danger-500' :
                      pin.status === 'planning' ? 'bg-primary-300' :
                      'bg-primary-600'
                    }`}>
                      {pin.status === 'alert' ? (
                    ...
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover:block z-10">
                      <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-2 text-xs whitespace-nowrap">
                        ...
                        {pin.alerts > 0 && <p className="text-danger-600">{pin.alerts}件 アラート</p>}
                      </div>
                    </div>
                  </div>
                )
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-4">
            <h3 className="text-sm font-semibold text-gray-700 mb-3">表示レイヤー</h3>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-100">
              <h3 className="font-bold text-gray-900 text-sm">エリアアラート</h3>
            </div>
                ...
                <div key={i} className="px-4 py-3">
                  <div className={`flex items-start gap-2`}>
                    <AlertTriangle className={`h-4 w-4 mt-0.5 flex-shrink-0 ${
                      alert.severity === 'alert' ? 'text-danger-500' :
                      alert.severity === 'warning' ? 'text-safety-500' :
                      'text-primary-500'
                    }`} />
                    <div>
                      <p className="text-xs font-semibold text-gray-800">{alert.site}</p>
          ...
          <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
            ...
            <div className="divide-y divide-gray-100">
              {equipment.map((eq) => {
                const status = equipmentStatus[eq.status as keyof typeof equipmentStatus]
                return (
                  <div key={eq.id} className="px-4 py-3 hover:bg-gray-50 transition-colors">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <p className="text-xs font-semibold text-gray-900">{eq.name}</p>
                        <p className="text-xs text-gray-500 mt-0.5">{eq.site}</p>
                        <p className="text-xs text-gray-400">操作: {eq.operator}</p>
                      </div>
          ...
          <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
            ...
            <div className="divide-y divide-gray-100">
              {sitePins.map((site) => (
                <div key={site.id} className="px-4 py-3 hover:bg-gray-50 transition-colors cursor-pointer">
                  <div className="flex items-center gap-3">
                    <MapPin className={`h-4 w-4 flex-shrink-0 ${
                      site.status === 'alert' ? 'text-danger-500' :
                      site.status === 'planning' ? 'text-gray-400' :
                      'text-primary-500'
                    }`} />
                    <div>
                      <p className="text-xs font-semibold text-gray-900">{site.name}</p>
                      {site.workers > 0 && (
                        <p className="text-xs text-gray-500">{site.workers}名 作業中</p>
                      )}
                    </div>
                    {site.alerts > 0 && (
